...
const VideoGridItem = ({
    ...
    const [isVideoPlaying, setIsViedoPlaying] = useState(false)
    ...
    return (
        <div className={ 'flex flex-col gap-2' }
             onMouseEnter={ () => setIsViedoPlaying(true) }
             onMouseLeave={ () => setIsViedoPlaying(false) }>
            <a href={ `/watch?v=${ id }` } className={ 'relative aspect-video' }>
                ...
                <video
                    className={ `block h-full object-cover absolute inset-0 transition-opacity duration-200 delay-200 ${ isVideoPlaying ? 'opacity-100 delay-100' : 'opacity-0' }` }
